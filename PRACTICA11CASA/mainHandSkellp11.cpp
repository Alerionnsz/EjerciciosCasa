...
//************************************************************//
//************************************************************//
//************** Alumno (s):PérezTepox Luis Carlos *********************************//
//*************		Grupo: 02	Práctica 11	Ejercicio de Casa							******//
//*************			Movimientos mano LIKE PIEDRA PAPEL Y TIJERA							******//
//************************************************************//
//************************************************************//
...
float posX = 0, posY = 2.5, posZ = -3.5, rotRodIzq = 0, rotRD = 0, rotArmR = 0, rotArmL = 0, rotHead = 0, giroM1 = 0, tijeras = 0, piedra = 0, like = 0, rotAnt=0;
...
typedef struct _frame
{
	...
	float giroM1;
	float giroM1Inc;
	float tijeras;  // para los movimientos de la manos
	float piedra;
	float like;
	float rotAnt;
	float tijerasInc;  // para los movimientos de la manos
	float piedraInc;
	float likeInc;
	float rotAntInc;
...
//CFiguras fig8;  // Para crear mano
CFiguras fig8;  // Para crear palma
...
void saveFrame ( void )
{
	...
	KeyFrame[FrameIndex].rotArmL = rotArmL;
	KeyFrame[FrameIndex].rotArmR = rotArmR;
	KeyFrame[FrameIndex].rotHead = rotHead;
	KeyFrame[FrameIndex].rotRodIzq=rotRodIzq;
	KeyFrame[FrameIndex].rotRD=rotRD;
	KeyFrame[FrameIndex].giroM1 = giroM1;
	KeyFrame[FrameIndex].tijeras = tijeras;
	KeyFrame[FrameIndex].piedra = piedra;
	KeyFrame[FrameIndex].like = like;
	KeyFrame[FrameIndex].rotAnt = rotAnt;
...
void resetElements( void )
{
	...
	rotArmL = KeyFrame[0].rotArmL;
	rotArmR = KeyFrame[0].rotArmR;
	rotHead = KeyFrame[0].rotHead;
	giroM1 = KeyFrame[0].giroM1;
	tijeras = KeyFrame[0].tijeras;
	like = KeyFrame[0].like;
	rotAnt = KeyFrame[0].rotAnt;
	piedra = KeyFrame[0].piedra;
...
void interpolation ( void )
{
	...
	KeyFrame[playIndex].rotRodDerInc = (KeyFrame[playIndex + 1].rotRD - KeyFrame[playIndex].rotRD) / i_max_steps;
	KeyFrame[playIndex].rotArmLInc = (KeyFrame[playIndex + 1].rotArmL - KeyFrame[playIndex].rotArmL) / i_max_steps;
	KeyFrame[playIndex].rotArmRInc = (KeyFrame[playIndex + 1].rotArmR - KeyFrame[playIndex].rotArmR) / i_max_steps;
	...
	KeyFrame[playIndex].giroM1Inc = (KeyFrame[playIndex + 1].giroM1 - KeyFrame[playIndex].giroM1) / i_max_steps;
	KeyFrame[playIndex].tijerasInc = (KeyFrame[playIndex + 1].tijeras - KeyFrame[playIndex].tijeras) / i_max_steps;
	KeyFrame[playIndex].rotAntInc = (KeyFrame[playIndex + 1].rotAnt - KeyFrame[playIndex].rotAnt) / i_max_steps;
	KeyFrame[playIndex].piedraInc = (KeyFrame[playIndex + 1].piedra - KeyFrame[playIndex].piedra) / i_max_steps;
	KeyFrame[playIndex].likeInc = (KeyFrame[playIndex + 1].like - KeyFrame[playIndex].like) / i_max_steps;
}
...
//FUNCION QUE DIBUJA UN PIRAMIDE RECTANGULAR
void piramide(float largo, float alto, float ancho)
{
	GLfloat vertice[8][3] = {
		{ 0.5*largo ,-0.5*alto, 0.5*ancho },    //Coordenadas Vértice 0 V0
		{ -0.5*largo ,-0.5*alto, 0.5*ancho },    //Coordenadas Vértice 1 V1
		{ -0.5*largo ,-0.5*alto, -0.5*ancho },    //Coordenadas Vértice 2 V2
		{ 0.5*largo ,-0.5*alto, -0.5*ancho },    //Coordenadas Vértice 3 V3
		{ 0.0*largo ,0.5*alto, 0.0*ancho },    //Coordenadas Vértice 4 V4
	};
	...
	glBegin(GL_POLYGON);	//Cara Tracera						
...
//para agregar otra cara de otro color
void piramide2(float largo, float alto, float ancho)
{
	GLfloat vertice[8][3] = {
		{ 0.5*largo ,-0.5*alto, 0.5*ancho },    //Coordenadas Vértice 0 V0
		{ -0.5*largo ,-0.5*alto, 0.5*ancho },    //Coordenadas Vértice 1 V1
...
void mano() {
	...
	glPushMatrix(); //Brazo derecho-->
			glTranslatef(-10.25, 0.65, 0);
			fig8.esfera(0.5, 12, 12, 0);
			glPushMatrix();
				glTranslatef(0.25, 0, 0);
				glRotatef(-45+rotArmR, 0, 0, 1); 
				glTranslatef(0.75, 0, 0);
				fig8.prisma(0.7, 1.5, 0.7, 0);
				glPushMatrix();//MUÑECA
					glDisable(GL_LIGHTING);
					glTranslatef(0.9, 0, 0);
					glColor3f(0,1,0);
					glRotatef(rotArmL, 0, 1, 0);
					fig8.prisma(0.7, 0.3, 0.7, 0);
					glEnable(GL_LIGHTING);
					...
					glPushMatrix();//PALMA
						...
						//glRotatef(45, 1, 0, 0);
						//glRotatef(rotArmL, 0, 1, 0);
						fig8.prisma(0.7, 0.5, 0.7, 0);
						glRotatef(-45 + rotArmR, 1, 0, 0);
					glEnable(GL_LIGHTING);
					glPopMatrix();//MEÑIQUE1.O
						glPushMatrix();
						glDisable(GL_LIGHTING);
						glTranslatef(0.90, -0.3, 0);
						glColor3f(0, 0.8, 0.8);
						fig8.prisma(0.2, 0.5, 0.2, 0);
						glRotatef(-45 + rotArmR, 1, 0, 0);
						glEnable(GL_LIGHTING);
						glPopMatrix();
						glPushMatrix();//MEÑIQUE1.1
						glDisable(GL_LIGHTING);
						glTranslatef(1.3, -0.3, 0);
						glColor3f(1, 0.8, 0.8);
						glRotatef(rotArmL, 0, 1, 0);
						fig8.prisma(0.2, 0.3, 0.2, 0);
						glRotatef(-45 + rotArmR, 1, 0, 0);
						glEnable(GL_LIGHTING);
						glPopMatrix();
						glPushMatrix();//MEÑIQUE1.2
						glDisable(GL_LIGHTING);
						glTranslatef(1.55, -0.3, 0);
						glColor3f(1, 0.0, 0.8);
						fig8.prisma(0.2, 0.2, 0.2, 0);
						glRotatef(-45 + rotArmR, 1, 0, 0);
						glRotatef(giroM1, 0, 1, 0);
						glEnable(GL_LIGHTING);
						glPopMatrix();
...
void palma() {
	//glDisable(GL_LIGHTING);
	glColor3f(1,0.8,0.8);
	glTranslatef(-15, 0 , 4); //posiciono la palma a manera que se vea toda la mano y a partir de ésta utilizar el eje de referencia
	glScalef(0.5, 0.5, 0.5);//para escalar toda la figura (palma y dedos)
	glRotatef(rotAnt, 0, 0, 1);

	//glRotatef(rotX, 1.0, 0.0, 0.0);//para rotar a partir del origen (centro de la palma) (posicionado ahora en -0.5, -1.5, 0.0)
	//glRotatef(rotY, 0.0, 1.0, 0.0);//para rotar a partir del origen (centro de la palma) (posicionado ahora en -0.5, -1.5, 0.0)
	//glRotatef(rotZ, 0.0, 0.0, 1.0);//para rotar a partir del origen (centro de la palma) (posicionado ahora en -0.5, -1.5, 0.0)
	glPushMatrix();
	glRotatef(rotAnt / 2, 0, 0, 1);
	fig8.prisma(5.0, 10, 2.0, 0);//dibujo antebraazo
	glPushMatrix(); // PALMA
	//glRotatef(0.0, 0.0, 0.0, 1.0);//giro para parar la mano
	//glRotated(0, 1.0, 0.0, 0.0);//giro para poner la palma hacia enfrente

	//--------------------------------- DEDO 4 (Índice) ----------------------------------

	glPushMatrix(); // Falange	
	glTranslatef(11.95, 2.7, 0.0);//traslación falange 
	glRotatef(piedra * 6.9, 0.0, 1.0, 0.0);		//GIRO FALANGE PIEDRA
	glRotatef(like * 6.9, 0.0, 1.0, 0.0);		//GIRO FALANGE PIEDRA
	glRotatef(-sciz*4.6, 0.0, 0.0, 1.0);					//GIRO FALANGE SEÑAL V
	glRotatef(-tijeras, 0.0, 0.0, 1.0);					//GIRO FALANGE TIJERA

	glPushMatrix(); // Falangina
	glTranslatef(2.0, 0.0, 0.0);
	glRotatef(piedra * 8.6, 0.0, 1.0, 0.0);		//GIRO FALANGINA SEÑAL ROCK
	glRotatef(like * 8.6, 0.0, 1.0, 0.0);		//GIRO FALANGINA SEÑAL ROCK
	...
	glRotatef(-piedra * 1.7, 1.0, 0.0, 0.0);		//GIRO FALANGETA SEÑAL ROCK
	glRotatef(-like * 1.7, 1.0, 0.0, 0.0);		//GIRO FALANGETA SEÑAL ROCK
	fig8.prisma(1.1, 1.5, 1.1, 0); // dibujo falangeta
	glPopMatrix(); // FIN FALANGETA

	glRotated(90, 0.0, 1.0, 0.0);
	fig8.prisma(1.3, 1.3, 2.0, 0); // dibujo falangina	
	glPopMatrix(); // FIN FALANGINA 

	glRotated(90, 0.0, 1.0, 0.0);
	fig8.prisma(1.5, 1.5, 2.0, 0); // dibujo falange
	glPopMatrix(); // FIN FALANGE 


	//--------------------------------- DEDO 3 (Medio) -----------------------------------
	glPushMatrix(); // Falange	
	glTranslatef(12.45, 0.9, 0.0);//traslación falange 
	glRotated(-90, 1.0, 0.0, 0.0); //giro anidado para cambiar color a todo el dedo 
	glRotatef(piedra * 11.7, 0.0, 0.0, 1.0);			//GIRO FALANGE
	glRotatef(like * 11.7, 0.0, 0.0, 1.0);			//GIRO FALANGE  
	glRotatef(-sciz * 5.6, 0.0, 1.0, 0.0);				//GIRO FALANGE 
	glRotatef(-tijeras, 0.0, 1.0, 0.0);				//GIRO FALANGE  

	glPushMatrix(); // Falangina
	glTranslatef(2.75, 0.0, 0.0);
	glRotatef(piedra * 8.6, 0.0, 0.0, 1.0);		//GIRO FALANGINA  
	glRotatef(like * 8.6, 0.0, 0.0, 1.0);		//GIRO FALANGINA  

	glPushMatrix(); // Falangeta
	glTranslatef(2.0, 0.0, 0.0);
	glRotatef(-90, 0.0, 0.0, 1.0);
	glRotatef(piedra * 1.9, 0.0, 0.0, 1.0);		//GIRO FALANGETA  
	glRotatef(like * 1.9, 0.0, 0.0, 1.0);		//GIRO FALANGETA  
	fig8.prisma(1.1, 1.5, 1.1, 0); // dibujo falangeta
	glPopMatrix(); // FIN FALANGETA

	glRotated(90, 0.0, 1.0, 0.0);
	fig8.prisma(1.3, 1.3, 2.5, 0); // dibujo falangina	
	glPopMatrix(); // FIN FALANGINA 

	glRotated(90, 0.0, 1.0, 0.0);
	fig8.prisma(1.5, 1.5, 3.0, 0); // dibujo falange
	glPopMatrix(); // FIN FALANGE 


	//-------------------------------- DEDO 2 (Anular) -----------------------------------
	glPushMatrix(); // Falange	
	glTranslatef(12.25, -0.9, 0.0);//traslación falange dedo 3
	glRotated(180, 1.0, 0.0, 0.0); //giro para cambiar color a todo el dedo (anidado)
	glRotatef(-piedra * 12, 0.0, 1.0, 0.0);				//GIRO FALANGE  
	glRotatef(-like * 12, 0.0, 1.0, 0.0);				//GIRO FALANGE  
	glRotatef(sciz*5.6, 0.0, 0.0, 1.0);				//GIRO FALANGE  
	glRotatef(-tijeras * 6, 0.0, 1.0, 0.0);				//GIRO FALANGE 
	glRotatef(-tijeras *0.2, 0.0, 0.0, 1.0);				//GIRO FALANGE  (TORCERLO)

	glPushMatrix(); // Falangina
	glTranslatef(2.5, 0.0, 0.0);
	glRotatef(-piedra *8.6, 0.0, 1.0, 0.0);		//GIRO FALANGINA 
	glRotatef(-like * 8.6, 0.0, 1.0, 0.0);		//GIRO FALANGINA  
	glRotatef(-tijeras * 4, 0.0, 1.0, 0.0);		//GIRO FALANGINA    
	glRotatef(-tijeras, 0.0, 0.0, 1.0);			//GIRO FALANGINA  (TORCERLO)

	glPushMatrix(); // Falangeta
	glTranslatef(1.95, 0.0, 0.0);
	glRotatef(-90, 0.0, 0.0, 1.0);
	glRotatef(piedra * 1.7, 1.0, 0.0, 0.0);		//GIRO FALANGETA 
	glRotatef(like * 1.7, 1.0, 0.0, 0.0);		//GIRO FALANGETA 
	glRotatef(tijeras, 1.0, 0.0, 0.0);			//GIRO FALANGETA 
	fig8.prisma(1.1, 1.5, 1.1, 0); // dibujo falangeta
	glPopMatrix(); // FIN FALANGETA

	glRotated(90, 0.0, 1.0, 0.0);
	fig8.prisma(1.3, 1.3, 2.4, 0); // dibujo falangina	
	glPopMatrix(); // FIN FALANGINA 

	glRotated(90, 0.0, 1.0, 0.0);
	fig8.prisma(1.5, 1.5, 2.6, 0); // dibujo falange
	glPopMatrix(); // FIN FALANGE 


	//-------------------------------- DEDO 1 (Meñique) -----------------------------------
	glPushMatrix(); // Falange	
	glTranslatef(11.7, -2.7, 0.0);//traslación falange 
	glRotated(90, 1.0, 0.0, 0.0); //giro para cambiar color a todo el dedo (anidado)
	glRotatef(-piedra * 12, 0.0, 0.0, 1.0);				//GIRO FALANGE 
	glRotatef(-like * 12, 0.0, 0.0, 1.0);				//GIRO FALANGE 
	glRotatef(-sciz * 4.6, 0.0, 1.0, 0.0);			//GIRO FALANGE 
	glRotatef(-tijeras * 6.7, 0.0, 0.0, 1.0);			//GIRO FALANGE 
	glRotatef(tijeras, 0.0, 1.0, 0.0);				//GIRO FALANGE 
	...
	glRotatef(-piedra * 8.6, 0.0, 0.0, 1.0);		//GIRO FALANGINA 
	glRotatef(-like * 8.6, 0.0, 0.0, 1.0);		//GIRO FALANGINA 
	glRotatef(-tijeras * 3, 0.0, 0.0, 1.0);			//GIRO FALANGINA 
	glRotatef(tijeras * 0.8, 0.0, 1.0, 0.0);		//GIRO FALANGINA 

	glPushMatrix(); // Falangeta
	glTranslatef(1.5, 0.0, 0.0);
	glRotatef(-90, 0.0, 0.0, 1.0);
	glRotatef(-piedra * 1.7, 0.0, 1.0, 0.0);		//GIRO FALANGETA 
	glRotatef(-like * 1.7, 0.0, 1.0, 0.0);		//GIRO FALANGETA 
	glRotatef(-tijeras, 0.0, 0.0, 1.0);		//GIRO FALANGETA 
	fig8.prisma(1.1, 1.5, 1.1, 0); // dibujo falangeta
	glPopMatrix(); // FIN FALANGETA

	glRotated(90, 0.0, 1.0, 0.0);
	fig8.prisma(1.3, 1.3, 1.5, 0); // dibujo falangina	
	glPopMatrix(); // FIN FALANGINA 

	glRotated(90, 0.0, 1.0, 0.0);
	fig8.prisma(1.5, 1.5, 1.5, 0); // dibujo falange
	glPopMatrix(); // FIN FALANGE 


	//-------------------------------- DEDO Gordo (Pulgar) -----------------------------------
	glPushMatrix(); // Falange	
	glTranslatef(9.1, 3, 0.0);//traslación falange dedo gordo
	glRotated(90, 0.0, 0.0, 1.0); //giro para cambiar color a todo el dedo y girarlo, ponerlo vertical (anidado)
	glRotatef(piedra *7, 0.0, 1.0, 0.0);				//GIRO FALANGE 
	glRotatef(-sciz * 1.1, 0.0, 0.0, 1.0);				//GIRO FALANGE 
	glRotatef(tijeras * 7, 0.0, 1.0, 0.0);			//GIRO FALANGE 

	glPushMatrix(); // Falangina
	glTranslatef(1.4, 0.0, 0.0);
	glRotatef(piedra *3, 0.0, 1.0, 0.0);		//GIRO FALANGINA 
	glRotatef(tijeras * 3, 0.0, 1.0, 0.0);			//GIRO FALANGINA 

	glPushMatrix(); // Falangeta
	glTranslatef(1.5, 0.0, 0.0);
	glRotatef(-90, 0.0, 0.0, 1.0);
	glRotatef(-piedra * 1.0, 0.0, 0.0, 0.0);		//GIRO FALANGETA 
	glRotatef(-tijeras, 1.0, 0.0, 0.0);			//GIRO FALANGETA 
	fig8.prisma(1.1, 1.5, 1.1,0); // dibujo falangeta
	glPopMatrix(); // FIN FALANGETA
	...
	fig8.prisma(1.5, 1.3, 1.3,0); // dibujo falangina	
	glPopMatrix(); // FIN FALANGINA 

	glRotated(180, 1.0, 0.0, 0.0);
	fig8.prisma(1.5, 1.5, 1.5, 0); // dibujo falange
	glPopMatrix(); // FIN FALANGE 
	glTranslatef(8.45, 0, 0);
	fig8.prisma(5.0, 6.9, 1.5, 0);//dibujo palma
	//glEnable(GL_LIGHTING);
	
	glColor3f(1, 1, 1);
	glPopMatrix();//fin antebrazo
	glPopMatrix(); // FIN PALMA
...
void InitGL ( GLvoid )     // Inicializamos parametros
{
	...
	for(int i=0; i<MAX_FRAMES; i++)
	{
		...
		KeyFrame[i].rotInc =0;
		KeyFrame[i].rotArmL = 0;
		KeyFrame[i].rotArmLInc = 0;
		KeyFrame[i].rotRD = 0;
		KeyFrame[i].rotRodDerInc = 0;
		KeyFrame[i].rotArmR = 0;
		KeyFrame[i].rotArmRInc = 0;
		KeyFrame[i].rotHead = 0;
		KeyFrame[i].rotHeadInc = 0;
		KeyFrame[i].giroMonito =0;
		KeyFrame[i].giroM1 = 0;
		KeyFrame[i].giroM1Inc = 0;
		KeyFrame[i].tijeras = 0;
		KeyFrame[i].tijerasInc = 0;
		KeyFrame[i].rotAntInc = 0;
		KeyFrame[i].rotAnt = 0;
		KeyFrame[i].piedra = 0;
		KeyFrame[i].piedraInc = 0;
		KeyFrame[i].like = 0;
		KeyFrame[i].likeInc = 0;
...
void animacion()
{
	...
	{		
		...
		else
		{
			...
			rotRD += KeyFrame[playIndex].rotRodDerInc;
			rotArmL += KeyFrame[playIndex].rotArmLInc;
			rotHead += KeyFrame[playIndex].rotHeadInc;
			rotArmR += KeyFrame[playIndex].rotArmRInc;
			tijeras += KeyFrame[playIndex].tijerasInc;
			rotAnt += KeyFrame[playIndex].rotAntInc;
			piedra += KeyFrame[playIndex].piedraInc;
			like += KeyFrame[playIndex].likeInc;
